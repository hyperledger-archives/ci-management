...
RELEASE_TAG=$(cat maven-metadata.xml | grep release | cut -d ">" -f2)
BINARY_TAG=$(echo $RELEASE_TAG | cut -d '<' -f1)
COMMIT=$(echo $BINARY_TAG | cut -d '-' -f3)
VERSION=$(cat fabric/Makefile | grep "BASE_VERSION =" | cut -d "=" -f2 | cut -d " " -f2)
